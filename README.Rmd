...
Add the data files and directory structure
...
Some `{ggplot}` styling files, not necessary for analysis but plotting functions will fail without them

```{r setup-source_files}
source("R/themes.R")
...
These functions aggregate and plot the total count of deaths. The only categories are the substance, poisoning or non-poisoning, and whether they were counted in the initial ONS data or added from analysis of the linked dataset:
...
```{r function-combine_national_data_drugs}
...
```{r function-get_national_data_drugs}
...
```{r function-relabel_national_data}
...
```{r function-plot_national_data_drugs}
...
```{r function-create_national_drugs_deaths_plot}
...
```{r function-combine_national_data_alcohol}
...
```{r function-aggregate_alcohol_deaths}
...
```{r function-plot_additional_alcohol_deaths}
...
```{r function-get_merged_national_data}
...
```{r function-plot_all_deaths_by_source}
...
```{r function-calculate_crude_yll, message=FALSE}
...
```{r function-plot_crude_yll_estimate, message=FALSE}
...
This function is from [here](https://static-content.springer.com/esm/art%3A10.1186%2F1471-2458-8-116/MediaObjects/12889_2007_1086_MOESM3_ESM.pdf)[^2]. It uses the same basic method but applies discounting and age-weighting.
...
```{r function-plot_substance_use_yll_estimate, message=FALSE}
