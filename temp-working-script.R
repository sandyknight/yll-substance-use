...
plot_leading_cause_comparison <- function(data){
data |> 
  arrange(count) |> 
  mutate(leading_cause = forcats::as_factor(leading_cause)) |> 
  ggplot(aes(x = count, y = leading_cause, fill = leading_cause)) + 
  geom_col() + 
  scale_fill_manual(values = c("Deaths associated with drug use" = "red", "Deaths associated with alcohol use" = "red", "Deaths associated with substance use" = "red")) + 
   my_theme +
